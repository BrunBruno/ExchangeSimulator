...
public class GetAllPreviousGamesTests : IClassFixture<TestWebApplicationFactory<Program>>
{
    ...
    public async Task GetAllPreviousGames_Should_Return_Games_On_Success()
    {
        ...
        //when
        var response1 = await _client.GetAsync("api/game/previous-games?gameName=&ownerName=&pageNumber=1&sortOption=0"); //sort by date
        var response2 = await _client.GetAsync("api/game/previous-games?gameName=&ownerName=&pageNumber=2&sortOption=0"); //sort by date
        var response3 = await _client.GetAsync("api/game/previous-games?gameName=&ownerName=&pageNumber=1&sortOption=1"); //sort by game name
        var response4 = await _client.GetAsync("api/game/previous-games?gameName=Game5&ownerName=&pageNumber=1&sortOption=1"); //sort by game name
